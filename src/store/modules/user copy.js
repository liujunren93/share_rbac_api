    ...
    Login ({ commit }, userInfo) {
      ...
      return new Promise((resolve, reject) => {
        login(userInfo, { success: false }).then(response => {
          ...
          storage.set(REFRESH_TOKEN, token.refresh_token, (token.expiry * 2 - token.created) * 1000)
    ...
    RefreshToken ({ commit, state }) {
      return new Promise((resolve, reject) => {
        ...
        if (state.token.refreshToken) {
          ...
          rtoken = storage.get(REFRESH_TOKEN)
        }
        if (rtoken) {
          refreshToken(rtoken).then(res => {
            const token = res.data.token
            storage.set(ACCESS_TOKEN, token.access_token, (token.expiry - token.created) * 1000)
            storage.set(REFRESH_TOKEN, token.refresh_token, (token.expiry - token.created) * 1000 * 2)
            commit('SET_TOKEN', token.access_token, token.refresh_token)
            resolve(res)
          }).catch(error => {
            reject(error)
